...
#define SCRIPT_BAD_CMD             -1
#define SCRIPT_INTERNAL_ERROR      -1
#define SCRIPT_ACCESS_ERROR        -2
#define SCRIPT_MEM_ERROR           -1
...
int set_part_name(char * name, int prt)
{
	...
	if( prt < MAX_PARTS_NUM )
	{
		...
		strncpy((char*)p->name,name,MAX_STR_LEN);
		p->name[MAX_STR_LEN - 1] = '\0';
...
int set_net_name(char * name, int prt)
{
	...
	if( prt < MAX_NETS_NUM )
	{
		...
		strncpy((char*)n->name,name,MAX_STR_LEN);
		n->name[MAX_STR_LEN - 1] = '\0';
...
int add_node(int x, int y, int netid)
{
	net * n;

	if( netid < MAX_NETS_NUM )
	{
		n =  (net *)&actx.nets[netid];
		if(n->nodes_cnt < MAX_NODES_NUM)
		{
			printf("net %d node %d: %d %d\n",netid, n->nodes_cnt,x,y);
			n->nodes_list[n->nodes_cnt].x = x;
			n->nodes_list[n->nodes_cnt].y = y;
			n->nodes_list[n->nodes_cnt].r = 10;
			n->nodes_cnt++;
		}
	}
...
int find_net(int x, int y, node * nodeskip)
{
	...
	for(i2=0;i2<actx.nets_cnt;i2++)
	{
		n =  (net *)&actx.nets[i2];

		for(j2=0;j2<n->nodes_cnt;j2++)
		{
			ni = &n->nodes_list[j2];

			if( isinnode( ni, x, y) )
			{
				if(!strlen((char*)actx.lastnetname))
				{
					strcpy((char*)actx.lastnetname,(char*)n->name);
				}
				else
					set_net_name(actx.lastnetname, i2);

				/*printf("Net : %d\n",i2);
				sprintf(tmpstr,"Net %s (NID %d)\n",n->name,i2);
				if(!strstr(&actx.connectionslist,tmpstr))
				{
					strcat((char*)&actx.connectionslist, tmpstr);
				}*/
				printf("Net : %d\n",i2);
				printf("Net %s (NID %d)\n",n->name,i2);
				/*if(!strstr(&actx.connectionslist,tmpstr))
				...
				// Node on pins - set all nodes as selected
				for(j3 = 0; j3<n->nodes_cnt;j3++)
				{
					ni = &n->nodes_list[j3];
					ni->s = 1;
					for(i4=0;i4<actx.parts_cnt+1;i4++)
					{
						p =  (part *)&actx.parts[i4];
						for(j4=0;j4<p->pins_cnt;j4++)
						{
							pi = &p->pins_list[j4];
							if( isinpin( pi, ni->x, ni->y ) )
							{
								// is there another net here ?
								if( nodeskip != ni && !pi->s )
									find_net( pi->x, pi->y, ni);

								pi->s=1;
								sprintf(tmpstr,"%s (PID %d) PIN %d\n",p->name,i4,j4+1);
								if(!strstr((char*)&actx.connectionslist,tmpstr))
								{
									strcat((char*)&actx.connectionslist, tmpstr);
								}
							}
	...
	for(i=0;i<actx.parts_cnt+1;i++)
	{
		...
		for(j=0;j<p->pins_cnt;j++)
		{
			...
			if( isinpin( pi, x, y ) )
			{
				...
				pi->s=1;
				sprintf((char*)&actx.lastsel,"Part ID %d: %s Pin %d",i,p->name,j+1);
				printf("Part / pin : %d (%s):%d\n",i,p->name,j);
				// Find the attached node(s)
				find_net(pi->x, pi->y, NULL);
			}
		}
	}
...
static int cmd_call( script_ctx * ctx, char * line )
{
	...
	if(offs>=0)
	{
		...
		new_ctx = init_script((void*)actx,0x00000000,(void*)actx->envvar);
		if(new_ctx)
		{
...
static int cmd_addpin( script_ctx * ctx, char * line)
{
	//addpin x y

	int i,j,ret;
	char varxpos[DEFAULT_BUFLEN];
	char varypos[DEFAULT_BUFLEN];

	ret = SCRIPT_CMD_BAD_PARAMETER;

	i = get_param( ctx, line, 1, varxpos );
	j = get_param( ctx, line, 2, varypos );

	if(i>=0 && j>=0)
	{
		add_pin(str_to_int(varxpos),str_to_int(varypos), actx.parts_cnt);
...
static int cmd_addpart( script_ctx * ctx, char * line)
{
	//add_part
...
static int cmd_partname( script_ctx * ctx, char * line)
{
	//set_partname
	int i,ret;
	...
	i = get_param_str( ctx, line, 1, name );

	if(i>=0)
	{
		set_part_name( (char*)name, actx.parts_cnt);
...
static int cmd_netname( script_ctx * ctx, char * line)
{
	//set_partname
	int i,ret;
	char name[DEFAULT_BUFLEN];
	...
	if(i>=0)
	{
		set_net_name( (char*)name, actx.nets_cnt);
...
static int cmd_addnode( script_ctx * ctx, char * line)
{
	//add_node x y
	...
	if(i>=0 && j>=0)
	{
		add_node(str_to_int(varxpos),str_to_int(varypos), actx.nets_cnt);
...
static int cmd_addnet( script_ctx * ctx, char * line)
{
	//add_net
...
cmd_list script_commands_list[] =
{
	...
	{"add_pin",                 cmd_addpin},
	{"add_node",                cmd_addnode},
	{"add_net",                 cmd_addnet},
	{"add_part",                cmd_addpart},
	{"set_partname",            cmd_partname},
	{"set_netname",             cmd_netname},
