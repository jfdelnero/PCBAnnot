...
///////////////////////////////////////////////////////////////////////////////////
// File : bmp_file.c
// Contains: bmp file loader/writer
//
...
int bmp_load(char * file,bitmap_data * bdata)
{
	...
	BITMAPFILEHEADER_HXC bitmap_header;
	BITMAPINFOHEADER_HXC bitmap_info_header;
	RGBQUAD_HXC * palette;
	uint32_t * data_ptr;
	...
	if(f)
	{
		fread(&bitmap_header,sizeof(BITMAPFILEHEADER_HXC),1,f);

		if( bitmap_header.bfType == 19778 )
		{
			fread(&bitmap_info_header,sizeof(BITMAPINFOHEADER_HXC),1,f);
			...
			if(!bitmap_info_header.biCompression)
			{
				...
				if(palettesize)
				{
					palette = (RGBQUAD_HXC *)malloc(palettesize*sizeof(RGBQUAD_HXC));
					if(!palette)
						goto error;

					memset(palette,0,palettesize*sizeof(RGBQUAD_HXC));

					fseek(f,sizeof(BITMAPFILEHEADER_HXC)+ bitmap_info_header.biSize ,SEEK_SET);
					fread(palette,palettesize*sizeof(RGBQUAD_HXC),1,f);
				}

				bitmapdataoffset = sizeof(BITMAPFILEHEADER_HXC)+ bitmap_info_header.biSize + (palettesize*sizeof(RGBQUAD_HXC));
...
int bmp24b_write(char * file,bitmap_data * bdata)
{
	...
	if(f)
	{
		memset(&bitmap_header,0,sizeof(BITMAPFILEHEADER_HXC));

		bitmap_header.bfType = 19778;
		bitmap_header.bfOffBits = sizeof(BITMAPFILEHEADER_HXC) + sizeof(BITMAPINFOHEADER_HXC);
		...
		bitmap_header.bfSize = sizeof(BITMAPFILEHEADER_HXC) + sizeof(BITMAPINFOHEADER_HXC) + (bitmapdatalinesize*bdata->ysize);
		fwrite(&bitmap_header,sizeof(BITMAPFILEHEADER_HXC),1,f);

		memset(&bitmap_info_header,0,sizeof(BITMAPINFOHEADER_HXC));
		bitmap_info_header.biSize = sizeof(BITMAPINFOHEADER_HXC);
		...
		fwrite(&bitmap_info_header,sizeof(BITMAPINFOHEADER_HXC),1,f);
...
int bmpRLE8b_write(char * file,bitmap_data * bdata)
{
	...
	if(f)
	{
		...
		bitmap_header.bfType = 19778;
		bitmap_header.bfOffBits = sizeof(BITMAPFILEHEADER_HXC) + sizeof(BITMAPINFOHEADER_HXC) + (256*4);
		...
		bitmap_header.bfSize = sizeof(BITMAPFILEHEADER_HXC) + sizeof(BITMAPINFOHEADER_HXC) + sizeof(pal) + (bitmapdatalinesize*bdata->ysize);
		fwrite(&bitmap_header,sizeof(BITMAPFILEHEADER_HXC),1,f);
		...
		bitmap_header.bfSize = sizeof(BITMAPFILEHEADER_HXC) + sizeof(BITMAPINFOHEADER_HXC) + sizeof(pal) + datasize;
		fseek(f,0,SEEK_SET);
		fwrite(&bitmap_header,sizeof(BITMAPFILEHEADER_HXC),1,f);

		bitmap_info_header.biSizeImage = datasize;
		fseek(f,sizeof(BITMAPFILEHEADER_HXC),SEEK_SET);
		fwrite(&bitmap_info_header,sizeof(BITMAPINFOHEADER_HXC),1,f);
